...
#SBATCH -o results/time_series-based_3sec.out
...
#### NOTE ####
# In this version, for each subject, we first do the grid search for hyper-parameters, 
# then we immediately use the best parameters to do the training and testing.
# For testing, we used Group G subject's data and Group Not G's data to replace target subject's data.
# IMPORTANT: To reduce the memory usage and not exceed the limit of the turing machine,
#            we do not store the grid_search list to store every subject model.

# Compare to V4.5, V5 treat all not_sync historical case as a new class,
# because no matter in which case, all the signals are coming from the same subject except they are not synchronously measured
...
csv_output = open("results/time_series-based_3sec.csv",'wt')
csv_output_binCase = open("results/time_series-based_3sec_binCase.csv",'wt')
csv_output_tacc = open("results/time_series-based_3sec_bestTrainAcc.csv", 'wt')
...
# Functions for generating 4 different classes of training data (ONLY FOR ONE SUBJECT)
...
#         return temp_data
    ...
    def make_class_0_data(self, subject, rawdata):        
        ...
        temp_y = np.zeros(temp_x.shape[0]) #labels
    ...
    def make_class_1_data(self, subject, rawdata,seg_altered_per_subject,option):
        ...
        for other_subject in other_subject_list:            
            for start in range(0,seg_altered_per_subject):
                ...
                if target_tmp_end - target_tmp_start == self.segment_len:
        ...
        temp_x = np.swapaxes(temp_x,0,1)
        temp_x = np.swapaxes(temp_x,1,2)
        temp_y = np.ones(temp_x.shape[0]) #labels
        self.append_data_labels(temp_x,temp_y)
    ...
    def make_class_2_data(self, subject, rawdata,seg_altered_per_subject,option):
        ...
        temp_x = np.swapaxes(temp_x,0,1)
        temp_x = np.swapaxes(temp_x,1,2)
        temp_y = np.full(int(temp_x.shape[0]),2) #labels
        self.append_data_labels(temp_x,temp_y)
...
#                 sig_idx_2.append(temp_sig_2)
        ...
        temp_x = np.swapaxes(temp_x,0,1)
        temp_x = np.swapaxes(temp_x,1,2)
        temp_y = np.full(int(temp_x.shape[0]),3) #labels
        self.append_data_labels(temp_x,temp_y)
    ...
    def make_hist_class_data_ECG(self, subject,rawdata_test,rawdata_hist):
        ...
        for start in range(0,int(len(rawdata_test[subject])/self.p.segment_len)):
            ...
            if hist_tmp_end - hist_tmp_start == self.segment_len:
...
class my_model(object):
    ...
    def __init__(self,Param):
        self.p = Param
#         self.param_grid = {'batch_size': [20,25,30,35,40,45,50,55,60],
#                            'epochs': [10,15,20,25,30,35,40],
#                            # 'optimizer':['adam','rmsprop','sgd','adamax','nadam'],
#                            'maxnorm_val':[5,6,7,8,9,10], 
#                            'num_neuron_fc':[50,60,70,80,90,100,110],
#                            'act_func':['relu','sigmoid','softmax','tanh'],
#                            'filt_num_lvl_1':[10,15,20,25,30,35,40,45,50],
#                            'filt_num_lvl_2':[5,10,15,20,25,30,35,40],
#                            'filt_shape_lvl_1':[(70,1), (80,1), (90,1), (100,1), (110,1), (120,1)],
#                            'filt_shape_lvl_2':[(30,1), (35,1), (40,1), (45,1), (50,1), (55,1)],
#                            'pool_size_lvl_1':[(5,1),(10, 1),(15,1),(20,1),(25,1),(30,1)],
#                            'pool_size_lvl_2':[(5,1),(10, 1),(15,1),(20,1),(25,1),(30,1)],
#                            'dropOut_rate_lvl_1':[0.05,0.1,0.15,0.2,0.25,0.3,0.35],
#                            'dropOut_rate_lvl_2':[0.05,0.1,0.15,0.2,0.25,0.3,0.35],
...
#                           }
        ...
        self.param_grid = {'batch_size': [50],
                           ...
                           'filt_num_lvl_1':[20],
                           'filt_num_lvl_2':[15],
                           'filt_shape_lvl_1':[(54,1)], #[(108,1)]
                           'filt_shape_lvl_2':[(27,1)], #[(54,1)]
                           'pool_size_lvl_1':[(10, 1)],
                           'pool_size_lvl_2':[(10, 1)],
    ...
    def create_ConvolutionalNN(self, epochs, maxnorm_val, num_neuron_fc, act_func1, act_func2, filt_num_lvl_1, filt_num_lvl_2, 
        ...
        model = Sequential()
        model.add(Conv2D(filt_num_lvl_1, filt_shape_lvl_1, input_shape=(self.p.segment_len,1,2),
...
for subject in parameters.subject_G_list:
    ...
    x_train = gen_train.timeseries_to_image(gen_train.data)
    ...
    x_test_G = gen_test_G.timeseries_to_image(gen_test_G.data)
    ...
    x_test_notG = gen_test_notG.timeseries_to_image(gen_test_notG.data)
    ...
    x_test_hist = gen_test_hist.timeseries_to_image(gen_test_hist.data)
    ...
    model_built  = KerasClassifier(build_fn = m.create_ConvolutionalNN, 
                                  ...
                                  filt_num_lvl_1 = 20,
                                  filt_num_lvl_2 = 15,
                                  filt_shape_lvl_1 = (54,1), 
                                  filt_shape_lvl_2 = (27,1), 
                                  pool_size_lvl_1 = (10, 1),
                                  pool_size_lvl_2 = (10, 1),
                                  dropOut_rate_lvl_1 = 0.2, 
                                  dropOut_rate_lvl_2 = 0.2,
    ...
    print('Historical Sync test (combined)')
    ...
    x_test_hist_2 = gen_test_hist_2.timeseries_to_image(gen_test_hist_2.data)
    ...
    for duration_diff in parameters.async_duration_diff:
        ...
        x_test_hist_3 = gen_test_hist_3.timeseries_to_image(gen_test_hist_3.data)
